...
impl StorageManager {
    ...
    pub async fn get_recent_calls(&self, tool_id: &str, limit: usize) -> Result<Vec<ToolCallRecord>> {
        let tool_id = tool_id.to_string();
    ...
    pub async fn aggregate_daily_stats(&self, date: &str) -> Result<()> {
        ...
        self.conn
            .call(move |conn| {
                // Compute p95 as: sort latencies ASC, OFFSET floor(N * 0.95).
                // If N * 0.95 < 1 the OFFSET clamps to the last index via the
                // MIN guard so the subquery always returns one row.
                conn.execute(
                    r#"
                    ...
                    SELECT
                        ...
                        COALESCE(
                            (
                                SELECT latency_ms
                                FROM tool_calls AS tc2
                                WHERE tc2.tool_id = tool_calls.tool_id
                                  AND tc2.success = 1
                                  AND date(tc2.called_at, 'unixepoch') = date(?1)
                                ...
                                OFFSET MAX(
                                    ...
                                    (
                                        ...
                                        FROM tool_calls AS tc3
                                        WHERE tc3.tool_id = tool_calls.tool_id
                                          AND tc3.success = 1
                                          AND date(tc3.called_at, 'unixepoch') = date(?1)
                                    ) - 1
                                )
                            ),
                            0.0
