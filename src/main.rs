...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::GenConfig { format, aggressive, output } => {
...
async fn start_server(config_path: PathBuf) -> anyhow::Result<()> {
    ...
    gateway::start_gateway(config, backend_manager, router, health_manager, Some(storage)).await?;

    Ok(())
}

async fn generate_report(config_path: PathBuf, output: Option<PathBuf>, days: u64) -> anyhow::Result<()> {
    let config = Config::load(&config_path)?;
    let storage = StorageManager::new(&config.storage.db_path).await?;
    let storage = std::sync::Arc::new(storage);
...
async fn list_tools(config_path: PathBuf, sort_by: &str) -> anyhow::Result<()> {
    ...
    println!("\n{:<50} {:>12} {:>10}", "Tool ID", "Health Score", "Status");
    println!("{}", "-".repeat(75));
...
async fn show_status(config_path: PathBuf) -> anyhow::Result<()> {
    ...
    let registered = storage.get_all_registered_tools().await.unwrap_or_default().len();
...
async fn gen_config(
    ...
    let suggestions = health::generate_cleanup_suggestions(
        &health_manager,
        Some(&storage),
        aggressive,
    )
