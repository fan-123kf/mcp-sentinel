...
pub async fn generate_health_report(
    ...
    let healthy_count = all_scores
        .iter()
        .filter(|s| !s.degraded && !s.zombie)
        .count();
    ...
    report.push_str(&format!(
        "- **Healthy**: {} ({:.1}%)\n",
        healthy_count,
        healthy_count as f64 / all_scores.len().max(1) as f64 * 100.0
    ));
    report.push_str(&format!(
        "- **Degraded**: {} ({:.1}%)\n",
        degraded_count,
        degraded_count as f64 / all_scores.len().max(1) as f64 * 100.0
    ));
    report.push_str(&format!(
        "- **Zombie**: {} ({:.1}%)\n",
        zombie_count,
        zombie_count as f64 / all_scores.len().max(1) as f64 * 100.0
    ));
    report.push_str(&format!(
        "- **Estimated wasted tokens/turn from zombies**: ~{}\n\n",
        zombie_count * 150
    ));
    ...
    if let Some(storage) = storage {
        ...
        for score in &all_scores {
            if let Ok(count) = storage
                .get_call_count_window(&score.tool_id, time_window_days)
                .await
            {
                if count > 0 {
                    if let Ok(p95) = storage.get_p95_latency(&score.tool_id, 1000).await {
                        if let Some(health) =
                            health_manager.get_detailed_health(&score.tool_id).await
                        {
    ...
    if !degraded.is_empty() {
        ...
        for deg in &degraded {
            if let Some(health) = health_manager.get_detailed_health(&deg.tool_id).await {
                ...
                report.push_str(&format!(
                    "| `{}` | {:.3} | {} | {} |\n",
                    deg.tool_id, deg.health_score, health.consecutive_failures, last_success_str
                ));
            }
        }
...
pub async fn generate_cleanup_suggestions(
    ...
    for score in &all_scores {
        ...
        let is_zombie = if let Some(storage) = storage {
            if let Ok(count) = storage
                .get_call_count_window(&score.tool_id, threshold_days)
                .await
            {
                count == 0
            } else {
