...
async fn handle_search_tools(state: &AppState, arguments: Value) -> Result<Value> {
    ...
    let mut results = state.router.search(&params.query, params.top_k, health_weight).await;
    ...
    let trace_id = state.traces.record_search(&params.query, &results, "hybrid_rrf").await;
...
async fn handle_invoke(state: &AppState, arguments: Value) -> Result<Value> {
    ...
    policy.authorize(params.confirmed)?;
    ...
    match result {
        ToolCallResult::Success { content, latency_ms } => Ok(json!({
            "status": "success",
            "result": content,
